...
std::vector<cv::Vec3b> KMeansClusteringHamerly::clusterValues(const cv::Mat &image, const size_t n_clusters,
    ...
    std::vector<cv::Vec3b> centroids = calcKppSeedCentroids(image, n_clusters, metric);
...
std::vector<cv::Vec3b> KMeansClusteringNaive::clusterValues(const cv::Mat &image, const size_t n_clusters,
    ...
    auto start = std::chrono::steady_clock::now();
    auto centroids = calcKppSeedCentroids(image, n_clusters, metric);
...
// These are the hyper optimized variants purely for one metric. I'll miss you guys.
/*
std::vector<cv::Vec3b> KMeansClusteringHamerly::clusterValues(const cv::Mat &image, const size_t n_clusters) {
    if (n_clusters == 0) {
        return std::vector<cv::Vec3b>();
