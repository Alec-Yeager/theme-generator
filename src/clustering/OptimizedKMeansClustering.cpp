...
std::vector<cv::Vec3b> OptimizedKMeansClusteringHamerly::clusterValues(const cv::Mat &image, const size_t n_clusters) {
    if (n_clusters == 0) {
    ...
    std::vector<cv::Vec3b> centroids = calcKppSeedCentroids(image, n_clusters);
    ...
    std::vector<int32_t> distances_last_moved = std::vector<int32_t>(n_clusters);
    std::vector<int32_t> nearest_centroid_distances = std::vector<int32_t>(n_clusters, INT32_MAX);

    // Point related vectors
    std::vector<size_t> assigned_clusters = std::vector<size_t>(image.total());
    std::vector<int32_t> distance_upper_bounds = std::vector<int32_t>(image.total());
    std::vector<int32_t> distance_lower_bounds = std::vector<int32_t>(image.total());
    ...
    auto pointAllCenters = [&](const int32_t &fx, const int32_t &fy, const int32_t &fz, const size_t &idx) {
        ...
        int32_t min_sq_dist = dx * dx + dy * dy + dz * dz;
        // impossibly large to make sure it gets overwritten later
        int32_t second_min_sq_dist = INT32_MAX;
        size_t min_idx = 0;
        ...
        for (int j = 1; j < centroids.size(); j++) {
            ...
            dy = (fy - centroids[j][1]);
            dz = (fz - centroids[j][2]);
            // Shortcut for the bit
            int32_t dist = dx * dx;
            if (dist > second_min_sq_dist) {
                continue;
            } else {
                dist += dy * dy + dz * dz;
            }
            if (dist < min_sq_dist) {
                second_min_sq_dist = min_sq_dist;
                min_sq_dist = dist;
                min_idx = j;
            } else {
                second_min_sq_dist = std::min(second_min_sq_dist, dist);
            }
        }
        assigned_clusters[idx] = min_idx;
        distance_upper_bounds[idx] = std::sqrt(min_sq_dist);
        distance_lower_bounds[idx] = std::sqrt(second_min_sq_dist);
    };
    ...
    while (!converged) {
        ...
        nearest_centroid_distances = std::vector<int32_t>(n_clusters, INT32_MAX);
        ...
        for (size_t j = 0; j < centroids.size(); ++j) {
            for (size_t jp = j + 1; jp < centroids.size(); ++jp) {
                ...
                int dx = cent1[0] - cent2[0];
                int dy = cent1[1] - cent2[1];
                int dz = cent1[2] - cent2[2];

                int32_t centroid_dist = std::sqrt(dx * dx + dy * dy + dz * dz);
        ...
        for (auto it = image.begin<cv::Vec3b>(); it != image.end<cv::Vec3b>(); ++it) {
            ...
            if (distance_upper_bounds[idx] > triangle_ineq) {
                ...
                int32_t dx = (fx - centroid[0]);
                int32_t dy = (fy - centroid[1]);
                int32_t dz = (fz - centroid[2]);

                distance_upper_bounds[idx] = std::sqrt(dx * dx + dy * dy + dz * dz);
        ...
        for (size_t j = 0; j < centroids.size(); ++j) {
            ...
            int32_t dx = (centroid_ref[0] - old_centroid[0]);
            int32_t dy = (centroid_ref[1] - old_centroid[1]);
            int32_t dz = (centroid_ref[2] - old_centroid[2]);

            distances_last_moved[j] = std::sqrt(dx * dx + dy * dy + dz * dz);
        }

        if (std::accumulate(distances_last_moved.begin(), distances_last_moved.end(), 0) == 0) {
            converged = true;
            break;
        }
        ...
        int32_t greatest_dist = distances_last_moved[0];
        int32_t second_greatest_dist = INT32_MAX;
        for (size_t j = 1; j < centroids.size(); ++j) {
            int32_t dist = distances_last_moved[j];
        ...
        for (size_t i = 0; i < image.total(); ++i) {
            ...
            if (greatest_moved_center == assigned_clusters[i]) {
                distance_lower_bounds[i] =
                    std::max(0, distance_lower_bounds[i] - distances_last_moved[second_greatest_moved_center]);
            } else {
                distance_lower_bounds[i] =
                    std::max(0, distance_lower_bounds[i] - distances_last_moved[greatest_moved_center]);
            }
        }
        iteration++;
    }
...
std::vector<cv::Vec3b> OptimizedKMeansClusteringNaive::clusterValues(const cv::Mat &image, const size_t n_clusters) {
    if (n_clusters == 0) {
        return std::vector<cv::Vec3b>();
    }
    // Create initial points.
    auto start = std::chrono::steady_clock::now();
    auto centroids = calcKppSeedCentroids(image, n_clusters);
...
std::vector<cv::Vec3b> OptimizedKMeansClustering::calcKppSeedCentroids(const cv::Mat &image, const size_t n_clusters) {
    ...
    while (centroids.size() < n_clusters) {
        ...
        for (auto it = image.begin<cv::Vec3b>(); it != image.end<cv::Vec3b>(); ++it) {
            ...
            int64_t dx = (fx - centroids[0][0]);
            int64_t dy = (fy - centroids[0][1]);
            int64_t dz = (fz - centroids[0][2]);

            int64_t min_dist = dx * dx + dy * dy + dz * dz;
            for (int j = 1; j < centroids.size(); j++) {
                dx = (fx - centroids[j][0]);
                dy = (fy - centroids[j][1]);
                dz = (fz - centroids[j][2]);
