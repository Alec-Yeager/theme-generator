...
void CylindricalColorAccumulator::add(cv::Vec3b val, bool include_count) {
    ...
    // We devided the angles earler by 2 to fit them in our 8 bit number, here we multiply back to ensure the sin/cos is
    // correct.
    angular_sum_[0] += std::sin(static_cast<double>(val[0]) * 2 * (M_PI / 180.0));
    angular_sum_[1] += std::cos(static_cast<double>(val[0]) * 2 * (M_PI / 180.0));
    linear_sum_[0] += val[1];
    linear_sum_[1] += val[2];
    if (include_count)
        count_++;
...
cv::Vec3b CylindricalColorAccumulator::mean(size_t n) {
    ...
    // Make sure to convert back to 180 degree range.
    return cv::Vec3b{static_cast<u_char>(std::round(mean_degrees / 2.0)),
